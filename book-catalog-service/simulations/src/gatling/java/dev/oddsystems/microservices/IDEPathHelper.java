package dev.oddsystems.microservices;
...
public class IDEPathHelper {
  ...
  static {
    try {
      URL gatlingConfUrl = IDEPathHelper.class.getResource("/gatling.conf");
      Path projectRootDir = Paths.get(requireNonNull(gatlingConfUrl, "Couldn't locate gatling.conf").toURI()).getParent().getParent().getParent().getParent();
